...
def run_boosting( X_train, X_test, y_train, y_test, dataset_name):
    print("Running Ada Boosting on dataset: ", dataset_name)

    classification_objective = AdaBoostClassifier
    custom_tuning_parameters = {'n_estimators': range(5, 100, 5)}
    custom_parameters = {'base_estimator': DecisionTreeClassifier(max_depth=1)}
    ...
    visualization.show("results/Ada_Boosting_learning_curve_{}.png".format(dataset_name))
    plt.clf()
...
def generate_validation_curve(model, param_name, param_range, scoring, cv, dataset_name, X_train, y_train):
    ...
    viz.show("results/Ada_Boosting_model_complexity_{}_{}.png".format( dataset_name, param_name))
    plt.clf()